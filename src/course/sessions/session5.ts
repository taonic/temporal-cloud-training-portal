...
/**
 * Session 5 — Troubleshooting Chaos Lab.
 *
 ...
 *
 * Drill numbering follows the original four-drill list, which included breaking
 * an encryption proxy this course no longer deploys. The numbers are left alone
 * rather than closed up: the runbooks and the escalation matrix reference them,
 * and renumbering a drill in a runbook is how a runbook stops being trusted. So
 * this session runs 1, 2 and 4, with no 3.
 */
...
export const session5: SessionDef = {
  ...
  note:
    ...
    'runbook you write afterwards, not the green tick. Workers here are unversioned — no Build ID, ' +
    'no Worker Deployment — which is what makes drill 1 possible at all: it is the same deployment ' +
    'with changed code underneath it, and nothing on the platform is watching for that.',

  labSteps: [
    `Drill 1 — non-determinism. Start a workflow with id ${DRILL_1_PREFIX}-<yourname> against \`uv run lab5_chaos.py worker\`, then restart that worker as \`uv run lab5_chaos.py worker --break-determinism\` — same deployment, changed code, which is exactly the mistake Worker Versioning exists to prevent. The workflow task starts failing on replay; the execution is stuck, not failed, and nothing alerts. Find it, then restart the worker unmodified and watch it complete.`,
    ...
    `Drill 4 — stuck Activity. Run \`uv run lab5_chaos.py stuck\`, which starts ${DRILL_4_PREFIX}-<yourname> with an Activity that heartbeats and then hangs. Use the heartbeat to tell "stuck" from "slow", then let the heartbeat timeout fire and watch the retry resume from the last reported progress.`,
    'Draft the two runbooks — "Workflow stuck" and "Non-determinism error" — in one document. Record it by adding a `"session-5" = "<your doc link>"` key to the `temporalcloud_namespace_tags "lab"` block already in `lab1.tf` — add to that block rather than writing a second one, because it manages the namespace\'s COMPLETE tag set and a second declaration would delete every tag the earlier sessions set.',
  ],
...
#                                       workflow id ready.`,
  ...
  checkpoints: [
    ...
    },
    {
      ...
      detail: 'Looks for a `session-5` namespace tag pointing at the two runbooks.',
      selfAttested: true,
    },
  ],
