...
/**
 * Session 3 — Observability & Operational Readiness, over BOTH metric sources.
 *
...
export const session3: SessionDef = {
  ...
  labCommands: ({ namespaceName, namespaceId, metricsAccountName }) => [
    ...
    },
    {
      ...
      command: `cd labs/worker\nuv run lab3_metrics.py worker --metrics-port ${METRICS_PORT}`,
      expect:
        `\`SDK metrics on http://localhost:${METRICS_PORT}/metrics\`, then \`Polling ` +
        `'${LAB_TASK_QUEUE}'\`. The workers in this course are unversioned — no Build ID, no ` +
        'Worker Deployment. Worth knowing before you adopt Worker Versioning: a versioned worker ' +
        'whose version nothing routes to is handed no tasks at all, with pollers present and no ' +
        'error anywhere, which looks exactly like this screen and behaves nothing like it.',
      grades: 'worker-polled',
    },
    {
      label: 'Generate traffic — one workflow is a dot, fifty is a curve',
      command: 'uv run lab3_metrics.py load --count 50',
      expect:
  ...
  snippetLang: 'hcl',
  snippet: ({ metricsAccountName, namespaceName }) => `# Goes in labs/lab3.tf.
#
...
#
#                        Worker (Lab 2)          Metrics scraper (Lab 3)
    ...
    steps: [
      ...
      },
      {
        ...
        expect:
          ...
          'at all, so without it this returns blank rather than zero. Run Session 5 drill 1 and it ' +
