...
/**
 * Session 7 — Troubleshooting Chaos Lab.
 *
 ...
 *
 * Drill 3 (break the encryption proxy) is parked with Session 4 — it needs that
 * session's proxy to break. Drill numbers still follow the original list, so this
 * session runs 1, 2 and 4 with no 3, and there are two runbooks rather than three.
 */
const DRILL_1_PREFIX = 'chaos-1';
const DRILL_4_PREFIX = 'chaos-4';

export const session7: SessionDef = {
  number: 7,
  ...
  labSteps: [
    ...
    `Drill 4 — stuck Activity. Run \`uv run main.py chaos stuck\`, which starts ${DRILL_4_PREFIX}-<yourname> with an Activity that heartbeats and then hangs. Use the heartbeat to tell "stuck" from "slow", then let the heartbeat timeout fire and watch the retry resume from the last reported progress.`,
    'Draft the two runbooks — "Workflow stuck" and "Non-determinism error" — in one document. Record it by adding a `"session-7" = "<your doc link>"` key to the `temporalcloud_namespace_tags "lab"` block already in `lab1.tf` — add to that block rather than writing a second one, because it manages the namespace\'s COMPLETE tag set and a second declaration would delete every tag the earlier sessions set.',
  ],
...
#                                       workflow id ready.`,
  ...
  checkpoints: [
    ...
    {
      id: 'session-7-documented',
      title: 'Runbooks and escalation path recorded',
      detail: 'Looks for a `session-7` namespace tag pointing at the two runbooks.',
      selfAttested: true,
    },
  ],
  ...
  async grade(ctx) {
    ...
    const tag = (namespaceTags(ns)['session-7'] ?? '').trim();
    const documented = ctx.check(
      'session-7-documented',
      tag !== '',
      `Documented at ${tag}`,
      'No session-7 tag on your namespace.',
    );
